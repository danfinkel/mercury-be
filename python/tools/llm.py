...
Example bash command:
    ...
    return response_parser(response.model_dump()) # type: ignore
...
def prompt_func(
    ...
    response = openai.chat.completions.create( # type: ignore
        model=model, messages=messages, tools=tools, tool_choice=tool_choice # type: ignore
    )
    ...
    return func_responses # type: ignore
...
def getTableDefs(DB_URL, DATABASE_SCHEMA):
    # connect to db using with statement and create a db_manager
    with PostgresDatabase(DB_URL, schema=DATABASE_SCHEMA) as db:
